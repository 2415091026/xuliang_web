...
<template>
  ...
  >
    <div class="relative mx-auto flex h-full w-full max-w-7xl items-center justify-between px-6 md:px-8">
      ...
      <div class="absolute inset-y-0 left-1/2 flex -translate-x-1/2 items-center justify-center gap-9 max-[760px]:hidden">
      ...
      <div v-if="userInfo" class="flex items-center">
        <el-dropdown
          ...
          :teleported="false"
          popper-class="custom-logout-dropdown"
        ...
        >
          ...
          <el-button class="user-menu-btn" aria-label="用户菜单">
            ...
            <!-- 悬浮箭头微动画 -->
            <el-icon
              class="text-[#fff8ea]/60 transition-transform duration-200"
...
.user-menu-btn {
  ...
  background-color: rgba(0, 0, 0, 0.38) !important; /* bg-black/38 */
  ...
  backdrop-filter: blur(24px) !important; /* backdrop-blur-xl */
...
.user-menu-btn:hover {
  border-color: rgba(242, 184, 75, 0.46) !important; /* hover:border-[#f2b84b]/46 */
  background-color: rgba(0, 0, 0, 0.48) !important; /* hover:bg-black/48 */
}
